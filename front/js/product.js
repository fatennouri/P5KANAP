...
if (productId !== null) {
  ...
  selectPanierButton.addEventListener("click", (event) => {
    ...
    if (
      ...
      Quantitychosen > 0 &&
      Quantitychosen <= 100 &&
      Number.isInteger(Quantitychosen)
    ) {
      ...
      const SendProducttoStorage = () => {
        ...
        const existingProduct = productputinLocalStorage.find(
          (product) =>
            product.productId === Productchoices.productId &&
            product.colorProduct === Productchoices.colorProduct
        );
        if (existingProduct) {
          const totalQuantity =
            Number(existingProduct.quantityProduct) +
            Number(Productchoices.quantityProduct);
          if (totalQuantity <= 100) {
            ...
            existingProduct.quantityProduct =
              Number(existingProduct.quantityProduct) +
              Number(Productchoices.quantityProduct);
            alert(
              `La quantité du produit ${chosenProduct.name} de couleur ${ColorChosen} a été mise à jour.`
            );
            ...
            LocalStorageNotification = false;
            const errorMessage =
              "La quantité d'un article ne peut pas dépasser 100. Merci de modifier la quantité demandée.";
            alert(errorMessage);
          }
        }
      ...
      let productputinLocalStorage = JSON.parse(
      ...
      if (LocalStorageNotification) {
        alert(
          `Le produit ${chosenProduct.name} de couleur ${ColorChosen} a bien été ajouté au panier.`
        );
      }
    }
    //si la couleur n'est pas choisie et/ou la quantité n'est pas valide(100>non valide<1)
      ...
      alert(
        `Veuillez sélectionner une couleur et une quantité comprise entre 1 et 100 (entier) pour ajouter le produit au panier !`
      );
      console.log(alert);
    }
  });
