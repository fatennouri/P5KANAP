...
function NumberofProducts() {
  TotalQuantity += parseInt(NumberofProductPanier);
...
function TotalProductsPrice() {
  ...
  TotalProductPricePanier = NumberofProductPanier * PriceofProductPanier;
...
function RecalculateTotalPrice() {
  ...
  for (const item of productputinLocalStorage) {
    ...
    const FindProducts = MyProduits.find(
...
function ModifiyQuantity() {
  ...
  ModifyQuantity.forEach((item) => {
    ...
    item.addEventListener("change", (event) => {
      ...
      if (
        ModifiedQuantity > 0 &&
        ModifiedQuantity <= 100 &&
        Number.isInteger(ModifiedQuantity)
      ) {
      ...
      if (messageErrorQuantity) {
        alert(
          "La quantité d'un article doit être un nombre entier compris entre 1 et 100 pour le même article de référence et de couleur. Veuillez modifier la quantité choisie pour respecter cette plage de valeurs.."
        );
      }
    });
  });
